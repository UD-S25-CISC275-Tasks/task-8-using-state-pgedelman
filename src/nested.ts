...
export function sumPoints(questions: Question[]): number {
    return questions.reduce(
        (prevValue: number, curQuestion: Question) =>
            prevValue + curQuestion.points,
        0,
    );
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (prevValue: number, curQuestion: Question) =>
            prevValue + (curQuestion.published ? curQuestion.points : 0),
        0,
    );
}
...
export function toCSV(questions: Question[]): string {
    return questions.reduce(
        (prevValue: string, curQuestion: Question) =>
            prevValue +
            `\n${curQuestion.id},${curQuestion.name},${curQuestion.options.length},${curQuestion.points},${curQuestion.published}`,
        "id,name,options,points,published",
    );
}
